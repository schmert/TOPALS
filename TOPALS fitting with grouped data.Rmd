...
&=
\begin{pmatrix} 
\boldsymbol{b}_0 \mu_0
& \cdots  
& \boldsymbol{b}_{A-1}\mu_{A-1}
\end{pmatrix} \boldsymbol{W}^\prime \\
...
$$
and the 
derivative of its transpose with respect to TOPALS parameters $\boldsymbol{\alpha}$ is a $G \times K$ matrix:
...
\begin{aligned}
  ...
  &= 
   ...
   \tfrac{1}{M_G}\tfrac{\partial M_G}{\partial \boldsymbol{\alpha}}  \right] \boldsymbol{D}_{obs} -  \boldsymbol{X}^\prime diag(\boldsymbol{N}) \boldsymbol{1}  -  \boldsymbol{P} \boldsymbol{\alpha}
 \\
&= \frac{\partial \boldsymbol{M}^\prime}{\partial \boldsymbol{\alpha}} diag(\frac{1}{\boldsymbol{M}}) \, \boldsymbol{D}_{obs} - \boldsymbol{X}^\prime \boldsymbol{N} 
...
## (preliminary & rudimentary) function to fit and plot a TOPALS schedule

fit_TOPALS = function(D,N) {
 ...
 for (i in 2:ncol(alpha)) {
   alpha[,i] = next_alpha( alpha[,i-1], D, N)
 } 
...
this_plot =
  ...
  print(this_plot)
  return(NULL)  # temporary -- will want detailed output
}

fit_TOPALS(bigD,bigN)
